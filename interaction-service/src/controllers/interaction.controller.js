...
exports.getCoursePosts = async (req, res) => {
  try {
    ...
    const [posts, total] = await Promise.all([ForumPost.find(filter).sort({
      createdAt: -1
    }).skip(skip).limit(limit).lean(), ForumPost.countDocuments(filter)]);
...
exports.getPostComments = async (req, res) => {
  try {
    ...
    const postExists = await ForumPost.exists({
      _id: postId
    });
    if (!postExists) {
      return res.status(404).json({
    ...
    const [comments, total] = await Promise.all([Comment.find(filter).sort({
      createdAt: 1
    }).skip(skip).limit(limit).lean(), Comment.countDocuments(filter)]);
    return res.status(200).json({
...
exports.getCommentReplies = async (req, res) => {
  try {
    ...
    const [replies, total] = await Promise.all([Comment.find(filter).sort({
      createdAt: 1
    }).skip(skip).limit(limit).lean(), Comment.countDocuments(filter)]);
    return res.status(200).json({
...
exports.deleteComment = async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      await Comment.deleteMany({
        _id: {
          $in: descendants.map(item => item._id)
        }
      }).session(session);
