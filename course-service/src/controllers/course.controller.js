...
function sanitizeThumbnailPayload(thumbnail, fallbackAltText = "") {
  ...
  const publicId = thumbnail.publicId ? String(thumbnail.publicId).trim() : null;
...
function sanitizeCourseCreationPayload(body = {}) {
  ...
  const thumbnail = sanitizeThumbnailPayload(allowedData.thumbnail, allowedData.title);
...
exports.addQuiz = async (req, res) => {
  try {
    ...
    const moduleExists = course.modules.some(module => String(module._id) === String(moduleId));
    if (!moduleExists) {
      return res.status(404).json({
...
exports.list = async (req, res) => {
  try {
    ...
    const [courses, total] = await Promise.all([Course.find(filter, projection).populate("categoryId", "name slug icon image parentCategory").sort(sort).skip((normalizedPage - 1) * normalizedLimit).limit(normalizedLimit).lean(), Course.countDocuments(filter)]);
    const normalizedCourses = courses.map(course => {
      const sponsorship = course.sponsorship;
      const hasActiveSponsorship = Boolean(sponsorship?.isSponsored && sponsorship?.startsAt && sponsorship?.endsAt && new Date(sponsorship.startsAt) <= now && new Date(sponsorship.endsAt) > now);
      return {
        ...course,
        sponsorship: {
          ...sponsorship,
...
exports.getOneForManagement = async (req, res) => {
  try {
    const course = await Course.findById(req.params.id).populate("categoryId", "name slug icon image parentCategory isActive");
    if (!course) {
      return res.status(404).json({
        message: "Cours introuvable."
      });
...
exports.listMine = async (req, res) => {
  try {
    ...
    const courses = await Course.find(filter).populate("categoryId", "name slug icon image parentCategory isActive").sort({
      updatedAt: -1
    });
    return res.status(200).json({
      courses,
...
exports.update = async (req, res) => {
  try {
    ...
    const forbiddenFields = ["_id", "instructorId", "modules", "quizzes", "sponsorship", "publishedAt", "archivedAt", "createdAt", "updatedAt"];
    const allowedData = {
      ...req.body
    };
    ...
    if (Object.prototype.hasOwnProperty.call(allowedData, "categoryId")) {
      allowedData.categoryId = await validateCourseCategory(allowedData.categoryId);
    }
    if (Object.prototype.hasOwnProperty.call(allowedData, "thumbnail")) {
      const thumbnail = sanitizeThumbnailPayload(allowedData.thumbnail, allowedData.title || course.title);
      if (thumbnail === undefined) {
        return res.status(400).json({
...
exports.addModule = async (req, res) => {
  try {
    ...
    course.modules.push({
      ...moduleData,
      order: moduleOrder,
      resources: Array.isArray(resources) ? resources.map(sanitizeResourcePayload) : []
    });
    await course.save();
...
exports.getPaymentDetails = async (req, res) => {
  try {
    ...
    if (String(course.status).toUpperCase() !== "PUBLISHED" || course.isActive === false) {
      return res.status(400).json({
...
exports.grantAccess = async (req, res, next) => {
  try {
    ...
    expiresAt.setMonth(expiresAt.getMonth() + Number(accessPlan.durationMonths));
    const features = typeof accessPlan.features?.toObject === "function" ? accessPlan.features.toObject() : {
      courseContent: accessPlan.features?.courseContent ?? true,
      forumAccess: accessPlan.features?.forumAccess ?? false,
      instructorMessaging: accessPlan.features?.instructorMessaging ?? false,
      personalizedFollowUp: accessPlan.features?.personalizedFollowUp ?? false,
      assignmentCorrection: accessPlan.features?.assignmentCorrection ?? false,
...
exports.publish = async (req, res) => {
  try {
    ...
    const hasActiveResource = activeModules.some(module => module.resources.some(resource => resource.isActive));
    if (!hasActiveResource) {
      return res.status(400).json({
        message: "Le cours doit contenir au moins une ressource active."
      });
    }
    if (!course.pricing?.isFree && !course.pricing?.accessPlans?.some(plan => plan.isActive)) {
      return res.status(400).json({
        message: "Un cours payant doit contenir au moins un plan d'accès actif."
      });
    }
...
exports.getStudentCourses = async (req, res) => {
  try {
    ...
    const courses = await Course.find({
      ...
      status: "PUBLISHED",
      isActive: true
    }).populate("categoryId", "name slug icon image parentCategory").lean();
...
exports.getStudentCourseById = async (req, res) => {
  try {
    ...
    const course = await Course.findOne({
      ...
      status: "PUBLISHED",
      isActive: true
    }).select(["_id", "title", "description", "categoryId", "level", "language", "tags", "instructorId", "thumbnail", "thumbnailAssetId", "modules", "quizzes", "publishedAt"].join(" ")).populate("categoryId", "name slug icon image parentCategory").lean();
    if (!course) {
      return res.status(404).json({
        message: "Cours introuvable ou indisponible."
      });
    }
    const modules = (course.modules || []).filter(module => module.isActive !== false).sort((a, b) => (a.order || 0) - (b.order || 0)).map(module => ({
      _id: module._id,
      title: module.title,
      description: module.description,
      order: module.order,
      resources: (module.resources || []).filter(resource => resource.isActive !== false).sort((a, b) => (a.order || 0) - (b.order || 0)).map(resource => ({
        _id: resource._id,
    ...
    const quizzes = (course.quizzes || []).filter(quiz => quiz.isActive !== false).sort((a, b) => (a.order || 0) - (b.order || 0)).map(quiz => ({
      _id: quiz._id,
      title: quiz.title,
      description: quiz.description,
      order: quiz.order
...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    for (const answer of answers) {
      ...
      const selectedOptionIds = Array.isArray(answer.selectedOptionIds) ? answer.selectedOptionIds.map(String) : [];
      answersByQuestionId.set(questionId, [...new Set(selectedOptionIds)]);
    }
    const quizQuestionIds = new Set(quiz.questions.map(question => String(question._id)));
    ...
    let earnedPoints = 0;
    const totalPoints = quiz.questions.reduce((total, question) => total + Number(question.points || 1), 0);
    const details = quiz.questions.map(question => {
      const questionId = String(question._id);
      const selectedOptionIds = answersByQuestionId.get(questionId) || [];
      const validOptionIds = new Set((question.options || []).map(option => String(option._id)));
      const containsInvalidOption = selectedOptionIds.some(optionId => !validOptionIds.has(optionId));
      const correctOptionIds = (question.options || []).filter(option => option.isCorrect === true).map(option => String(option._id));
      const normalizedSelectedOptionIds = [...selectedOptionIds].sort();
      const normalizedCorrectOptionIds = [...correctOptionIds].sort();
      const isCorrect = !containsInvalidOption && normalizedSelectedOptionIds.length === normalizedCorrectOptionIds.length && normalizedSelectedOptionIds.every((optionId, index) => optionId === normalizedCorrectOptionIds[index]);
    ...
    const answeredQuestions = details.filter(detail => detail.isAnswered).length;
    const correctAnswers = details.filter(detail => detail.isCorrect).length;
    return res.status(200).json({
      message: passed ? "Quiz réussi avec succès." : "Quiz terminé, mais la note de passage n’a pas été atteinte.",
      result: {
        courseId: String(course._id),
...
exports.instructorDashboard = async (req, res) => {
  try {
    ...
    const courses = await Course.find({
      instructorId
    }).populate("categoryId", "name slug icon image").sort({
      updatedAt: -1
    }).lean();
    const courseIds = courses.map(course => course._id);
    const enrollments = courseIds.length ? await CourseEnrollment.find({
      courseId: {
        $in: courseIds
      }
    }).sort({
    ...
    const activeEnrollments = enrollments.filter(enrollment => enrollment.status === "ACTIVE" && (!enrollment.expiresAt || new Date(enrollment.expiresAt) > new Date()));
    const totalModules = courses.reduce((total, course) => total + (Array.isArray(course.modules) ? course.modules.length : 0), 0);
    const totalResources = courses.reduce((total, course) => total + (Array.isArray(course.modules) ? course.modules.reduce((moduleTotal, module) => moduleTotal + (Array.isArray(module.resources) ? module.resources.length : 0), 0) : 0), 0);
    const normalizedCourses = courses.map(course => ({
      id: course._id,
      title: course.title,
      description: course.description,
      ...
      resourcesCount: course.modules?.reduce((total, module) => total + (module.resources?.length || 0), 0) || 0,
    ...
    return res.status(200).json({
      stats: {
        ...
        archivedCourses: courses.filter(course => course.status === "ARCHIVED").length,
...
exports.adminSummary = async (req, res) => {
  try {
    const [totalCourses, published, drafts, archived, recentCourses] = await Promise.all([Course.countDocuments(), Course.countDocuments({
      status: "PUBLISHED"
      ...
      createdAt: -1
    }).limit(5).select("title status thumbnail instructor createdAt").lean()]);
    return res.json({
      stats: {
        totalCourses,
...
exports.adminList = async (req, res) => {
  try {
    ...
    const courses = await Course.find(filter).populate("categoryId", "name").sort({
      createdAt: -1
