...
function sanitizePayload(body = {}) {
  const { _id, createdAt, updatedAt, ...allowedData } = body;
  return allowedData;
}
...
exports.update = async (req, res) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "parentCategory")) {
      payload.parentCategory = await validateParentCategory(
...
exports.remove = async (req, res) => {
  try {
    ...
    const [courseCount, childCount] = await Promise.all([
      Course.countDocuments({ categoryId: category._id }),
