...
const resourceSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ["VIDEO", "ARTICLE", "PDF", "DOCUMENT", "IMAGE", "ZIP", "AUDIO", "QUIZ", "EXERCISE"],
    required: true
  },
  order: {
    type: Number,
...
quizQuestionSchema.pre("validate", function validateQuizQuestion(next) {
  try {
    ...
    if (correctOptions.length === 0) {
      return next(new Error(`La question "${this.question}" doit contenir au moins une bonne réponse.`));
    }
    if (["SINGLE_CHOICE", "TRUE_FALSE"].includes(this.type) && correctOptions.length !== 1) {
      return next(new Error(`Une question de type ${this.type} doit avoir exactement une bonne réponse.`));
    }
    if (this.type === "TRUE_FALSE" && options.length !== 2) {
      return next(new Error("Une question de type TRUE_FALSE doit contenir exactement deux options."));
    }
    next();
  } catch (error) {
    next(error);
...
resourceSchema.pre("validate", function validateResource(next) {
  try {
    if (this.type === "VIDEO" && (!this.video?.url || !this.video?.publicId)) {
      return next(new Error("Une ressource VIDEO doit contenir une vidéo valide."));
    }
    if (["PDF", "DOCUMENT", "ZIP", "AUDIO"].includes(this.type) && (!this.file?.url || !this.file?.publicId)) {
      return next(new Error(`Une ressource ${this.type} doit contenir un fichier valide.`));
    }
    if (this.type === "IMAGE" && (!this.image?.url || !this.image?.publicId)) {
      return next(new Error("Une ressource IMAGE doit contenir une image valide."));
    }
...
courseSchema.pre("validate", function validateCourse(next) {
  try {
    ...
    if (pricing.isFree) {
      for (const accessPlan of accessPlans) {
        if (Number(accessPlan.price) !== 0) {
          return next(new Error("Tous les prix doivent être égaux à 0 lorsque le cours est gratuit."));
        }
      }
    } else if (accessPlans.length === 0) {
      return next(new Error("Un cours payant doit contenir au moins une offre tarifaire."));
    }
    const planCombinations = new Set();
    for (const accessPlan of accessPlans) {
      const key = `${accessPlan.planType}-${accessPlan.durationMonths}`;
      if (planCombinations.has(key)) {
        return next(new Error(`Le plan ${accessPlan.planType} de ${accessPlan.durationMonths} mois existe déjà.`));
      }
    ...
    if (sponsorship.isSponsored && (!sponsorship.startsAt || !sponsorship.endsAt || !sponsorship.sponsorshipId)) {
      return next(new Error("Un cours sponsorisé doit avoir un identifiant, une date de début et une date de fin."));
    }
    if (sponsorship.startsAt && sponsorship.endsAt && sponsorship.endsAt <= sponsorship.startsAt) {
      return next(new Error("La date de fin du sponsoring doit être postérieure à la date de début."));
    }
...
courseSchema.methods.getAccessPlan = function getAccessPlan(planType, durationMonths) {
  const normalizedPlanType = String(planType).toUpperCase();
  const normalizedDuration = Number(durationMonths);
  return this.pricing.accessPlans.find(accessPlan => accessPlan.isActive && accessPlan.planType === normalizedPlanType && accessPlan.durationMonths === normalizedDuration);
};
courseSchema.methods.hasActiveSponsorship = function hasActiveSponsorship() {
  const now = new Date();
  const sponsorship = this.sponsorship;
  return Boolean(sponsorship?.isSponsored && sponsorship.startsAt && sponsorship.endsAt && sponsorship.startsAt <= now && sponsorship.endsAt > now);
};
courseSchema.methods.clearExpiredSponsorship = function clearExpiredSponsorship() {
  const sponsorship = this.sponsorship;
  if (sponsorship?.isSponsored && sponsorship.endsAt && sponsorship.endsAt <= new Date()) {
...
courseSchema.methods.getOrderedContent = function getOrderedContent() {
  return this.modules.filter(module => module.isActive).sort((firstModule, secondModule) => {
    return firstModule.order - secondModule.order;
  }).map(module => ({
    ...module.toObject(),
    resources: module.resources.filter(resource => resource.isActive).sort((firstResource, secondResource) => {
