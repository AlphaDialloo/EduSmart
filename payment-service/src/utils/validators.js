...
function create(body = {}) {
  ...
  const accessPlanId = body.accessPlanId ? String(body.accessPlanId).trim() : null;
  ...
  if (paymentType === "INSTRUCTOR_MEMBERSHIP") {
    if (!validateUuid(referenceId)) {
      errors.push("referenceId doit être un UUID valide pour une adhésion formateur.");
    }
    if (accessPlanId) {
      errors.push("accessPlanId ne doit pas être fourni pour une adhésion formateur.");
    }
  }
  if (paymentType === "COURSE_PURCHASE") {
    if (!isMongoObjectId(referenceId)) {
      errors.push("referenceId doit être un ObjectId MongoDB valide pour l'achat d'un cours.");
    }
    if (!accessPlanId) {
