...
function validatePlatformSettings(payload) {
  ...
  if ("platformName" in payload) {
    if (
      ...
      payload.platformName.trim().length > 100
    ) {
      errors.push(
        "platformName doit contenir entre 2 et 100 caractères.",
      );
    }
  }
  ...
  if ("defaultLanguage" in payload) {
    if (!SUPPORTED_LANGUAGES.includes(payload.defaultLanguage)) {
      errors.push(
        `defaultLanguage doit être l'une des valeurs suivantes : ${SUPPORTED_LANGUAGES.join(", ")}.`,
      );
    }
  }
  ...
  if ("supportedLanguages" in payload) {
    if (
      ...
      payload.supportedLanguages.length === 0 ||
      payload.supportedLanguages.some(
        (language) => !SUPPORTED_LANGUAGES.includes(language),
      )
    ) {
      errors.push(
        `supportedLanguages doit être un tableau non vide contenant uniquement : ${SUPPORTED_LANGUAGES.join(", ")}.`,
      );
    }
  }
  ...
  if ("currency" in payload) {
    if (!SUPPORTED_CURRENCIES.includes(payload.currency)) {
      errors.push(
        `currency doit être l'une des valeurs suivantes : ${SUPPORTED_CURRENCIES.join(", ")}.`,
      );
    }
  }
  ...
  if ("timezone" in payload) {
    if (
      ...
      payload.timezone.trim().length < 3 ||
      payload.timezone.trim().length > 100
    ) {
  ...
  if ("defaultTheme" in payload) {
    if (!SUPPORTED_THEMES.includes(payload.defaultTheme)) {
      errors.push(
        `defaultTheme doit être l'une des valeurs suivantes : ${SUPPORTED_THEMES.join(", ")}.`,
      );
    }
  }
...
function validateFeatureFlag(payload, partial = false) {
  ...
  if (!partial || "key" in payload) {
    if (
      ...
      !/^[a-z0-9._-]{2,100}$/.test(payload.key)
    ) {
      errors.push(
        "key doit contenir entre 2 et 100 caractères minuscules, chiffres, points, tirets ou underscores.",
      );
    }
  }
  ...
  if (
    ...
    errors.push(
      "description doit être une chaîne de 1000 caractères maximum.",
    );
  }
