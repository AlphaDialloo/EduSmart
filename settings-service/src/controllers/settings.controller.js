...
exports.updateSettings = async (req, res) => {
  try {
    ...
    const settings = await repository.updatePlatformSettings(req.body, req.user.id);
    return res.status(200).json({
...
exports.getFeatureFlag = async (req, res) => {
  try {
    const featureFlag = await repository.getFeatureFlagByKey(req.params.key);
    if (!featureFlag) {
      return res.status(404).json({
...
exports.createFeatureFlag = async (req, res) => {
  try {
    ...
    const featureFlag = await repository.createFeatureFlag(req.body, req.user.id);
...
exports.updateFeatureFlag = async (req, res) => {
  try {
    ...
    const featureFlag = await repository.updateFeatureFlag(req.params.key, req.body, req.user.id);
    if (!featureFlag) {
      return res.status(404).json({
        message: "Fonctionnalité introuvable."
      });
...
exports.listCountryMembershipSettings = async (req, res) => {
  try {
    ...
    const countryMembershipSettings = await repository.listCountryMembershipSettings({
      enabledOnly
    });
    return res.status(200).json({
...
exports.getCountryMembershipSettings = async (req, res) => {
  try {
    const countryMembershipSettings = await repository.getCountryMembershipSettings(req.params.countryCode);
...
exports.createCountryMembershipSettings = async (req, res) => {
  try {
    ...
    const countryMembershipSettings = await repository.createCountryMembershipSettings(req.body, req.user.id);
    return res.status(201).json({
...
exports.updateCountryMembershipSettings = async (req, res) => {
  try {
    const errors = validateCountryMembershipSettings(req.body, true);
    if (errors.length > 0) {
      return res.status(400).json({
        message: "Données invalides.",
        errors
    ...
    const countryMembershipSettings = await repository.updateCountryMembershipSettings(req.params.countryCode, req.body, req.user.id);
    if (!countryMembershipSettings) {
      return res.status(404).json({
        message: "Tarification d'adhésion introuvable pour ce pays."
      });
...
exports.deleteCountryMembershipSettings = async (req, res) => {
  try {
    const deleted = await repository.deleteCountryMembershipSettings(req.params.countryCode);
    if (!deleted) {
      return res.status(404).json({
        message: "Tarification d'adhésion introuvable pour ce pays."
      });
