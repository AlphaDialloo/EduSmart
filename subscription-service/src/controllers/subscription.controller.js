...
async function createPendingSubscription(req, res, eventName) {
  try {
    ...
    const planCode = String(
      req.body.planCode || "INSTRUCTOR_ANNUAL",
    ).toUpperCase();

    const [plan, pricing] = await Promise.all([
      repository.getPlanByCode(planCode),
      settingsService.getCountryMembershipPricing(countryCode),
    ]);

    if (!plan) {
      return res.status(404).json({ message: "Plan d'adhésion introuvable." });
    }
    ...
    return res.status(result.created ? 201 : 200).json({
      message: result.created
        ? `${eventName} créée. Le paiement doit maintenant être traité.`
        : "Une demande de paiement est déjà en attente.",
...
exports.createSubscription = (req, res) =>
  createPendingSubscription(req, res, "Demande d'adhésion");

exports.renewSubscription = (req, res) =>
  createPendingSubscription(req, res, "Demande de renouvellement");
...
exports.cancelPending = async (req, res) => {
  try {
    const subscription = await repository.cancelPending(
      req.params.id,
      req.user.id,
    );
...
exports.checkInstructorActive = async (req, res) => {
  try {
    const subscription = await repository.getCurrentActive(
...
exports.paymentFailed = async (req, res) => {
  try {
    const subscription = await repository.markPaymentFailed(
      req.params.id,
...
exports.listAdmin = async (req, res) => {
  try {
    ...
    if (status && !SUBSCRIPTION_STATUSES.includes(status)) {
      return res.status(400).json({ message: "Filtre status invalide." });
    }
    ...
    const result = await repository.listSubscriptions({
      ...
      countryCode: req.query.countryCode
        ? normalizeCountryCode(req.query.countryCode)
        : undefined,
    });
...
exports.getAdminById = async (req, res) => {
  try {
    const [subscription, history] = await Promise.all([
      repository.getSubscriptionById(req.params.id),
      repository.getHistory(req.params.id),
    ]);
    ...
    return res.status(200).json({ subscription, history });
  } catch (error) {
    return sendError(res, error);
  }
};
...
exports.updateAdminStatus = async (req, res) => {
  try {
    ...
    const subscription = await repository.updateAdminStatus(
      req.params.id,
