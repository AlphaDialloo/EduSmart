...
async function getCountryMembershipPricing(countryCode) {
  try {
    const response = await client.get(
      `/api/settings/countries/${encodeURIComponent(countryCode)}/membership`,
    );
    ...
    if (!pricing) {
      const error = new Error(
        "Le settings-service n'a retourné aucune tarification.",
      );
    ...
    if (error.response?.status === 404) {
      const notFoundError = new Error(
        "Aucun tarif d'adhésion n'est configuré pour ce pays.",
      );
    ...
    const serviceError = new Error(
      "Impossible de récupérer le tarif d'adhésion.",
    );
