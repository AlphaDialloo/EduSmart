...
exports.listReflections = async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT id, enrollment_id, course_id, module_id, module_title,
             summary, confidence_level, created_at, updated_at
      FROM progress_service.learning_reflections
      WHERE enrollment_id = $1 AND user_id = $2
      ORDER BY updated_at DESC
    `, [enrollmentId, req.user.id]);
    return res.status(200).json({ reflections: result.rows, total: result.rowCount });
  } catch (error) {
...
exports.saveReflection = async (req, res) => {
  try {
    ...
    const moduleTitle = String(req.body.moduleTitle || "").trim();
    const summary = String(req.body.summary || "").trim();
    const confidenceLevel = Number(req.body.confidenceLevel || 3);
    if (!moduleTitle || moduleTitle.length > 200) {
      return res.status(400).json({ message: "Le titre de la notion est obligatoire et limité à 200 caractères." });
    }
    if (summary.length < 10 || summary.length > 4000) {
      return res.status(400).json({ message: "Le résumé doit contenir entre 10 et 4 000 caractères." });
    }
    if (!Number.isInteger(confidenceLevel) || confidenceLevel < 1 || confidenceLevel > 5) {
      return res.status(400).json({ message: "Le niveau de confiance doit être compris entre 1 et 5." });
    }
    const enrollmentResult = await pool.query(`
      SELECT id, course_id
      FROM progress_service.enrollments
    ...
    const result = await pool.query(`
      INSERT INTO progress_service.learning_reflections (
        enrollment_id, user_id, course_id, module_id, module_title,
        summary, confidence_level, created_at, updated_at
      )
      ...
      RETURNING *
    `, [enrollmentId, req.user.id, enrollmentResult.rows[0].course_id, String(moduleId), moduleTitle, summary, confidenceLevel]);
    return res.status(200).json({
      message: "Résumé de compréhension enregistré.",
...
exports.enroll = async (req, res) => {
  try {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [req.user.id, String(courseId).trim(), String(courseTitle).trim()]);
...
exports.resourceProgress = async (req, res) => {
  ...
  try {
    ...
    if (!Number.isInteger(normalizedTotalResources) || normalizedTotalResources < 1) {
      return res.status(400).json({
        message: "totalResources doit être un entier supérieur ou égal à 1."
      });
    }
    const normalizedProgress = completed ? 100 : normalizePercentage(progressPercentage);
    ...
    const resourceResult = await client.query(`
        ...
        RETURNING *
      `, [enrollmentId, String(moduleId), String(resourceId), normalizedProgress, Boolean(completed)]);
  ...
  `, [enrollmentId, overallProgress]);
    ...
    return res.status(200).json({
      ...
      progressSummary: {
        ...
        completedResources: Number(progressResult.rows[0].completed_resources || 0),
        overallProgress
      }
    });
  } catch (error) {
...
exports.quiz = async (req, res) => {
  ...
  try {
    ...
    const currentAttemptCount = Number(attemptsResult.rows[0].attempt_count) || 0;
    ...
    const insertResult = await client.query(`
        ...
        RETURNING *
      `, [req.user.id, enrollmentId, String(courseId), moduleId ? String(moduleId) : null, String(quizId), normalizedScore, normalizedPassingScore, passed, Number(earnedPoints) || 0, Number(totalPoints) || 0, Number(correctAnswers) || 0, Number(incorrectAnswers) || 0, Number(totalQuestions) || 0, JSON.stringify(details), attemptNumber, submittedAt]);
    const summaryResult = await client.query(`
      ...
      `, [req.user.id, String(courseId), String(quizId)]);
    await client.query("COMMIT");
    const attemptCount = Number(summaryResult.rows[0].attempt_count) || attemptNumber;
    const bestScore = Number(summaryResult.rows[0].best_score) || normalizedScore;
    return res.status(201).json({
...
exports.addLearningTime = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(normalizedDuration) || normalizedDuration < 1 || normalizedDuration > 300) {
      return res.status(400).json({
    ...
    const result = await pool.query(`
        ...
        RETURNING *
      `, [req.user.id, enrollmentId, String(courseId), moduleId ? String(moduleId) : null, resourceId ? String(resourceId) : null, normalizedDuration]);
    return res.status(201).json({
  ...
  try {
    ...
    return res.status(200).json({
      stats: {
        ...
        weeklyLearningSeconds: Number(learningTime.weekly_learning_seconds) || 0
      },
  ...
  try {
    ...
    const result = await pool.query(`
        ...
        DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP
        RETURNING *
      `, [String(studentId).trim(), String(courseId).trim(), String(courseTitle).trim()]);
    return res.status(201).json({
