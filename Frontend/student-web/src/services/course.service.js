...
const COURSE_API_URL =
  import.meta.env.VITE_COURSE_API_URL || "http://localhost:3000/api/courses";
...
function getLowestActivePlan(course) {
  const plans = Array.isArray(course?.pricing?.accessPlans)
    ? course.pricing.accessPlans.filter((plan) => plan?.isActive !== false)
    : [];
    ...
    (moduleTotal, module) =>
      ...
      (module?.resources || []).reduce(
        (resourceTotal, resource) =>
          resourceTotal + Number(resource?.durationMinutes || 0),
        0,
      ),
    0,
  );
...
export function normalizeCourse(course = {}) {
  ...
  return {
    ...
    price:
      course?.pricing?.isFree === true ? 0 : Number(lowestPlan?.price || 0),
    ...
    reviewCount: Number(
      course?.stats?.reviewCount || course?.stats?.reviews || 0,
    ),

    studentsCount: Number(
      course?.stats?.studentsCount || course?.stats?.students || 0,
    ),
