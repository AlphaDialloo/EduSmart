...
function CheckoutPage() {
  ...
  // Affichage seulement.
  // Le backend détermine lui-même le vrai montant du plan.
  ...
  const validateCheckout = () => {
    ...
    const missingField = requiredFields.find(
      (field) => !String(billing[field] || "").trim(),
    );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      /*
       * Le backend actuel crée un paiement par cours.
       * On traite donc les articles un par un.
       */
      for (const item of items) {
        ...
        const paymentId =
          creationResult?.payment?.id;

        if (!paymentId) {
          throw new Error(
            `Le paiement du cours « ${item.title} » n’a pas été créé correctement.`,
          );
        }

        /*
         * En mode TEST, cette requête passe le paiement
         * à SUCCEEDED, accorde l’accès au cours et crée
         * l’inscription dans le progress-service.
         */
      ...
      setSuccessMessage(
        "Paiement confirmé. Vos cours sont maintenant accessibles.",
      );
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f7f8fc]">
        <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
          ...
          {successMessage && (
            <div className="mt-7 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-bold text-emerald-700">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_390px]">
            <section className="space-y-8">
                ...
                <div className="mt-7 grid gap-5 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleBillingChange}
                      required
                      className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 uppercase outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
                    />
                  </div>
                </div>
              </article>
              ...
              <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                <label
                  className={`mt-7 flex cursor-pointer items-center gap-4 rounded-2xl border p-5 transition ${
                    paymentMethod === "TEST"
            ...
            <aside className="lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50">
                ...
                <button
                  ...
                  disabled={processing}
                  className="mt-7 flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-6 py-4 font-black text-white shadow-lg shadow-indigo-200 transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {processing ? (
                    <LoaderCircle
                      size={20}
