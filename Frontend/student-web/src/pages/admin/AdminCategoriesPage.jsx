...
const empty = { name:"", slug:"", description:"", icon:"", image:null, parentCategory:"", order:0, isActive:true };

export default function AdminCategoriesPage(){
  const {token}=useAuth();
  const [categories,setCategories]=useState([]), [form,setForm]=useState(empty), [editingId,setEditingId]=useState(null);
  const [loading,setLoading]=useState(true), [saving,setSaving]=useState(false), [error,setError]=useState("");
  const load=useCallback(async()=>{try{setLoading(true);setError("");const d=await getAdminCourseCategories(token);setCategories(d.categories||[]);}catch(e){setError(e.response?.data?.message||"Chargement impossible.");}finally{setLoading(false);}},[token]);
  useEffect(()=>{if(token)load();},[token,load]);
  const change=(e)=>{const {name,value,type,checked}=e.target;setForm(c=>({...c,[name]:type==="checkbox"?checked:value}));};
  const reset=()=>{setEditingId(null);setForm(empty);};
  const edit=(c)=>{setEditingId(c._id||c.id);setForm({name:c.name||"",slug:c.slug||"",description:c.description||"",icon:c.icon||"",image:c.image||null,parentCategory:c.parentCategory?._id||"",order:c.order||0,isActive:c.isActive!==false});window.scrollTo({top:0,behavior:"smooth"});};
  const submit=async(e)=>{e.preventDefault();try{setSaving(true);setError("");const payload={...form,name:form.name.trim(),slug:form.slug.trim()||undefined,description:form.description.trim()||null,icon:form.icon.trim()||null,parentCategory:form.parentCategory||null,order:Number(form.order)||0}; editingId?await updateCourseCategory(token,editingId,payload):await createCourseCategory(token,payload);reset();await load();}catch(err){setError(err.response?.data?.message||err.message||"Enregistrement impossible.");}finally{setSaving(false);}};
  const toggle=async(c)=>{try{await updateCourseCategoryStatus(token,c._id||c.id,c.isActive===false);await load();}catch(e){setError(e.response?.data?.message||"Action impossible.");}};
  const remove=async(c)=>{if(!window.confirm(`Supprimer ${c.name} ?`))return;try{await deleteCourseCategory(token,c._id||c.id);await load();}catch(e){setError(e.response?.data?.message||"Suppression impossible.");}};
