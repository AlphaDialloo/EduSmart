...
export default function CourseQuizzesTab({
  ...
  const submit = async event => {
    ...
    await onAdd({
      ...
      questions: [{
        ...
        order: 1,
        options: [{
          text: form.optionA.trim(),
          isCorrect: form.correct === "A"
        }, {
          text: form.optionB.trim(),
          isCorrect: form.correct === "B"
        }]
      }],
      isActive: true
    });
  ...
  return <div className="grid gap-7 xl:grid-cols-[420px_minmax(0,1fr)]">
      <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
        ...
        <div className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-bold">Module<select required value={form.moduleId} onChange={e => setForm(c => ({
            ...c,
          ...
          <label className="block text-sm font-bold">Bonne réponse<select value={form.correct} onChange={e => setForm(c => ({
            ...c,
            correct: e.target.value
          }))} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3"><option value="A">Option A</option><option value="B">Option B</option></select></label>
        </div>
        <button disabled={busy} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-4 font-black text-white disabled:opacity-60">{busy ? <LoaderCircle size={18} className="animate-spin" /> : <Plus size={18} />} Créer le quiz</button>
      </form>

      <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
        <h2 className="text-2xl font-black">Quiz du cours</h2>
        <div className="mt-6 space-y-4">
          {!quizzes?.length && <div className="py-12 text-center"><HelpCircle size={45} className="mx-auto text-emerald-600" /><p className="mt-4 font-bold text-slate-500">Aucun quiz.</p></div>}
          {(quizzes || []).map(quiz => <article key={quiz._id} className="flex items-center gap-4 rounded-2xl border border-slate-200 p-5">
