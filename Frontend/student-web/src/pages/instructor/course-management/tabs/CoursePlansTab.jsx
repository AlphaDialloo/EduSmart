...
export default function CoursePlansTab({ course, onSave, saving }) {
  ...
  useEffect(() => {
    ...
    setPlans(
      (course?.pricing?.accessPlans || []).map((plan) => ({
        _id: plan._id,
  ...
  const updatePlan = (index, key, value) => {
    setPlans((current) => current.map((plan, currentIndex) => currentIndex === index ? { ...plan, [key]: value } : plan));
  };
  ...
  const submit = () => {
    onSave({
      pricing: {
        ...
        accessPlans: isFree
          ? [{ planType: "STANDARD", durationMonths: 12, price: 0, isActive: true }]
          : plans.map((plan) => ({
              ...plan,
              durationMonths: Number(plan.durationMonths),
              price: Number(plan.price),
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button onClick={submit} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 font-black text-white disabled:opacity-60">
      ...
      {!isFree && (
        <div className="mt-7 space-y-4">
          {plans.map((plan, index) => (
            <div key={plan._id || index} className="grid gap-4 rounded-2xl border border-slate-200 p-5 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto_auto] lg:items-end">
          ...
          <button type="button" onClick={() => setPlans((current) => [...current, newPlan()])} className="inline-flex items-center gap-2 rounded-2xl border border-indigo-200 px-5 py-3 font-black text-indigo-600"><Plus size={18} /> Ajouter un plan</button>
        </div>
      )}
    </section>
  );
