import { Archive, ArrowLeft, BookOpen, FileText, HelpCircle, Info, LoaderCircle, Settings2, TrendingUp } from "lucide-react";
...
const tabs = [
  ...
  { id: "modules", label: "Modules & ressources", icon: BookOpen },
...
function InstructorCourseManagementPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [courseResponse, quizResponse] = await Promise.all([
        getInstructorCourseById(token, courseId),
  ...
  useEffect(() => { if (token && courseId) load(); }, [token, courseId, load]);
  ...
  if (loading) return <><Navbar /><main className="flex min-h-[70vh] items-center justify-center bg-[#f7f8fc]"><LoaderCircle size={44} className="animate-spin text-indigo-600" /></main></>;
      ...
      <main className="min-h-screen bg-[#f7f8fc]">
        <section className="border-b border-slate-200 bg-white">
          <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
            <Link to="/instructor/courses" className="inline-flex items-center gap-2 text-sm font-black text-indigo-600"><ArrowLeft size={17} /> Mes cours</Link>
            <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex min-w-0 items-center gap-5">
                ...
                <div className="min-w-0"><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-black text-indigo-600">{statusLabel(course.status)}</span><h1 className="mt-3 truncate text-3xl font-black text-slate-950">{course.title}</h1><p className="mt-1 text-sm text-slate-500">{course.modules?.length || 0} module(s) · {quizzes.length} quiz</p></div>
              </div>
        ...
        <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
          ...
          <nav className="mb-7 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2">
            {tabs.map((tab) => { const Icon = tab.icon; return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`inline-flex shrink-0 items-center gap-2 rounded-xl px-4 py-3 text-sm font-black ${activeTab === tab.id ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}><Icon size={17} />{tab.label}</button>; })}
          </nav>
          ...
          {activeTab === "statistics" && <CourseStatisticsTab course={{...course, quizzes}} />}
        </div>
      </main>
    </>
  );
