  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        const [response, recommendationData] = await Promise.all([
          getStudentDashboard(token),
          getDashboardRecommendations(token).catch(() => ({ recommendations: [] })),
        ]);
        ...
        console.error(
          "Erreur chargement dashboard étudiant :",
          requestError,
        );
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
                  ...
                  return (
                    ...
                    >
                      ...
                      <Link
                        ...
                        className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 font-black text-white"
                      >
                        <PlayCircle size={19} />
                        Continuer
                      </Link>
        ...
        <section className="mt-12">
          ...
          {recommendations.length === 0 ? (
            <div className="mt-6 rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">Terminez un cours pour recevoir des recommandations.</div>
          ) : (
            <div className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {recommendations.map((item) => {
                ...
                return <article key={item.id || courseId} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                  ...
                  <div className="p-5"><h3 className="text-xl font-black">{item.courseTitle || item.course_title || "Cours recommandé"}</h3><p className="mt-2 text-sm text-slate-500">{item.reason}</p><Link to={`/courses/${courseId}`} className="mt-5 inline-flex rounded-2xl bg-indigo-600 px-5 py-3 font-black text-white">Voir le cours</Link></div>
                </article>;
              })}
            </div>
          )}
